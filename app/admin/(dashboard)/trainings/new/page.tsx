...
export default function NewTrainingPage() {
  ...
  const handleSave = async (status: "draft" | "published") => {
    ...
    if (!form.startDate) return alert("La date de début est requise");
    setSaving(true);
    const res = await fetch("/api/admin/trainings", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ ...form, formFields, status }),
    });
    ...
    <div className="mx-auto max-w-3xl">
      <div className="mb-8 flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <h1 className="text-[28px] font-bold text-white">Nouvelle formation</h1>
        </div>
        ...
        <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-6">
          ...
          <div className="space-y-4">
            <div>
              ...
              <input value={form.titleFr} onChange={(e) => set("titleFr", e.target.value)} placeholder="Titre de la formation" className="w-full rounded-lg border border-white/[0.08] bg-white/[0.03] px-4 py-3 text-[14px] text-white placeholder-gray-600 outline-none focus:border-[#1F6FEB]/50" />
            </div>
            <div>
              <label className="mb-1.5 block text-[12px] font-medium text-gray-500">Description</label>
              <textarea value={form.descriptionFr} onChange={(e) => set("descriptionFr", e.target.value)} rows={4} placeholder="Description de la formation en français" className="w-full resize-y rounded-lg border border-white/[0.08] bg-white/[0.03] px-4 py-3 text-[14px] text-white placeholder-gray-600 outline-none focus:border-[#1F6FEB]/50" />
            </div>
          </div>
        </div>
        ...
        <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-6">
          ...
          <div className="space-y-4">
            <div>
              ...
              <input value={form.titleEn} onChange={(e) => set("titleEn", e.target.value)} placeholder="Training title" className="w-full rounded-lg border border-white/[0.08] bg-white/[0.03] px-4 py-3 text-[14px] text-white placeholder-gray-600 outline-none focus:border-[#1F6FEB]/50" />
            </div>
            <div>
              <label className="mb-1.5 block text-[12px] font-medium text-gray-500">Description</label>
              <textarea value={form.descriptionEn} onChange={(e) => set("descriptionEn", e.target.value)} rows={4} placeholder="Training description in English" className="w-full resize-y rounded-lg border border-white/[0.08] bg-white/[0.03] px-4 py-3 text-[14px] text-white placeholder-gray-600 outline-none focus:border-[#1F6FEB]/50" />
            </div>
          </div>
        </div>
