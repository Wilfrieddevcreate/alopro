...
const trainingStatusColors: Record<string, { bg: string; text: string; label: string }> = {
    ...
    <div>
      ...
      <div className="mb-6 flex gap-2">
        {(["all", "upcoming", "ongoing", "past"] as const).map((f) => {
          const labels = { all: "Toutes", upcoming: "À venir", ongoing: "En cours", past: "Terminées" };
          return (
            <button
              key={f}
              onClick={() => setFilter(f)}
      ...
      {!loading && filtered.length === 0 && (
        <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] py-16 text-center">
          <p className="text-[14px] text-gray-500">
            {filter === "all" ? "Aucune formation. Ajoutez votre première formation !" : "Aucune formation dans cette catégorie."}
          </p>
        </div>
      )}
      ...
      {!loading && filtered.length > 0 && (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((training) => {
            ...
            const ts = trainingStatusColors[getTrainingStatus(training.startDate)];
            return (
              <div
                key={training.id}
              ...
              >
                ...
                <div className="relative h-40 bg-white/[0.03]">
                  {training.image ? (
                    <img src={training.image} alt={training.titleFr} className="h-full w-full object-cover" />
                  ) : (
                  ...
                  <div className="absolute right-2 top-2 flex gap-1.5">
                    <span className={`rounded-full px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider ${ts.bg} ${ts.text}`}>
                      {ts.label}
                    </span>
                    <button
                ...
                <div className="p-4">
                  <p className="mb-0.5 text-[12px] text-gray-500">
                    {new Date(training.startDate).toLocaleDateString("fr-FR", { day: "numeric", month: "long", year: "numeric" })}
                  </p>
                  <h3 className="mb-1 text-[15px] font-bold text-white">{training.titleFr}</h3>
                  <p className="text-[12px] text-gray-500">{training.titleEn}</p>
                  ...
                  <div className="mt-3 flex flex-wrap gap-2 border-t border-white/[0.06] pt-3">
                    ...
                    <div className="relative">
                      <button
                        onClick={() => setShareOpen(shareOpen === training.id ? null : training.id)}
                      ...
                      {shareOpen === training.id && (
                        <div className="absolute bottom-full left-0 z-10 mb-2 w-48 rounded-lg border border-white/[0.08] bg-[#0a0a0a] p-2 shadow-xl">
                          {[
                            { name: "Facebook", url: `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(`${typeof window !== "undefined" ? window.location.origin : ""}/departments/training`)}`, color: "#1877F2" },
                            { name: "LinkedIn", url: `https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(`${typeof window !== "undefined" ? window.location.origin : ""}/departments/training`)}`, color: "#0A66C2" },
                            { name: "WhatsApp", url: `https://wa.me/?text=${encodeURIComponent(`${training.titleFr} — ${typeof window !== "undefined" ? window.location.origin : ""}/departments/training`)}`, color: "#25D366" },
                            { name: "X / Twitter", url: `https://twitter.com/intent/tweet?url=${encodeURIComponent(`${typeof window !== "undefined" ? window.location.origin : ""}/departments/training`)}&text=${encodeURIComponent(training.titleFr)}`, color: "#000" },
                          ].map((s) => (
                            <a
                              key={s.name}
                              href={s.url}
